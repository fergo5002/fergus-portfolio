import type { Article } from "../articles";

export const acceleratorWorth: Article = {
  slug: "what-an-accelerator-is-for",
  title: "What an accelerator is actually for",
  description:
    "One of nine from about 1,700 applicants, ten weeks in, company wound down. What the accelerator was genuinely worth, and what it could never have fixed.",
  date: "2026-08-04",
  tags: ["Startups", "Accelerators", "Founders"],
  summary:
    "An honest account of what a startup accelerator provides and what it cannot: forced deadlines and access are real value, product direction and founder alignment are not things a programme can supply. Written from the experience of a company that went through one and still wound down.",
  body: `I got into Hatch105 as one of nine from around 1,700 applicants. The company I took through it doesn't exist any more.

That makes me a reasonable person to ask whether it was worth it, because I have no programme to promote and no outcome to protect.

Short answer: yes, and not for any of the reasons in the brochure.

## What it genuinely gave us

**A deadline that wasn't ours.** This is most of the value and it's the least discussed.

Left alone, founders extend. There's always a reason the demo should be next week instead. An external date you didn't choose and can't move compresses decisions that would otherwise drift for a month. We went from first commit to live merchant brands in six weeks. We would not have done that on our own timeline and I'm certain of it, because I've now watched myself work on both.

**Access that would otherwise cost months.** Not the headline mentors. The unglamorous introductions: someone who's dealt with the same payments provider, someone who can tell you in five minutes what a compliance process actually involves rather than what the documentation claims. That compounds daily.

**Being around people at the same stage.** Underrated. The specific value is calibration. You find out that the thing you thought was uniquely hard is normal, and the thing you thought was going fine is not going fine. You don't get that from advisors, who are being kind, or from the internet, which is performing.

**Credibility.** Being one of nine from 1,700 opens doors, and I'd be pretending if I said otherwise. It's a real asset, it's still useful to me now, and it costs you nothing to carry.

## What it could not do

**It could not fix founder-product fit.** Three of us, pulling in different directions, for ten weeks. That's [why the company ended](/writing/why-presterly-wound-down).

No programme can solve that. Nobody in the building knows your co-founders well enough to see it, and by the time it's visible from outside it's visible because the company has already stopped working. The people running it were sharp and available. It wasn't their job and they couldn't have done it.

**It could not tell us the economics were wrong.** We didn't look properly until about week eight. That was ours to check. Nobody asks you to open the spreadsheet, because everyone assumes you did in week one.

**It could not make us talk to enough customers.** There's an assumption that a programme forces customer development. It creates the expectation. It cannot make you spend Tuesday on the phone instead of in the codebase, and given a choice between selling and building, I will build. That's a thing about me, not about the programme, and I've stopped expecting external structure to fix it.

## The thing people get wrong about applying

The application isn't a test of your idea. Everyone's idea is roughly equally unproven at that stage and the people reading know it.

It's a test of whether you can explain something clearly and whether you seem like someone who'll still be doing this in six months. The ideas that get in are not obviously better than the ones that don't. The founders are more legible.

Practical version: be specific and be honest about what you haven't done. Padding a thin traction slide reads instantly to anyone who's seen a hundred of them, and it costs you more credibility than the empty slide would have. "We have no revenue and here's exactly what we'd need to learn to get some" is a stronger answer than a chart with a made-up curve.

## Would I do another one

Not right now, and the reasoning is what I'd want someone else to think about.

The value of a programme is highest when the constraint is **speed, access or credibility**. Early, unknown, unsure how fast you can move.

My constraint isn't any of those any more. I can build fast, I've got the credential, and I know roughly who to ask. My constraint is commercial, and specifically that I keep choosing to own the technical half and hoping the other half is handled. Twice now.

A programme won't fix that. It's arguably the wrong tool, because it gives you a structure to be busy inside, and being busy is exactly how I've avoided the problem both times.

## If you're deciding

Ask what your actual constraint is, then ask whether a programme addresses it.

If you need to move faster than you will alone, a programme is very good value. If you need a specific door opened, and the programme opens that door, likewise.

If your problem is that you and your co-founder want different companies, or that you don't know if anyone will pay, no programme is going to help. Those get solved by a hard conversation and by talking to customers, both of which are free, available today, and much less pleasant than filling in an application.

I'd still do it again for Presterly. It compressed ten weeks of learning into ten weeks, which is precisely what it says it does. It just doesn't say, and can't say, that the ten weeks might teach you the company shouldn't exist.

That's a real outcome and it's worth having early.`,
};
